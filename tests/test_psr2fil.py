...
class TestFits2Fil(unittest.TestCase):
    ...
    def test_fits2fil_basic(self):
        ...
        np.testing.assert_array_equal(data_fil, data_fits)  # Account for channel flip
...
class TestFil2Fits(unittest.TestCase):
    ...
    def test_fil2fits_basic(self):
        ...
        test_fist_data = test_fits.get_data(0, test_header.nspectra, pol=0)
        out_fits_data = out_fits.get_data(0, out_header.nspectra, pol=0)
        # np.testing.assert_array_equal(data_fil, data_fits)
        np.testing.assert_array_equal(test_fist_data, out_fits_data)
